...
public class ExtendedOccurrence2AvroPipeline {
  ...
  public static void main(String[] args) {
    ...
    ExtendedOccurrenceTransform extendedOccurrenceTransform =
        ExtendedOccurrenceTransform.create().withAvroCoders(p);
    ...
    PCollection<ExtendedRecord> extendedRecordCollection =
        uniqueTuple.get(uniquenessTransform.getDataTag());

    // STEP 3: Run the main transform
    PCollectionTuple temporalRecordTuple =
        extendedRecordCollection.apply(extendedOccurrenceTransform);
    PCollection<ExtendedOccurrence> temporalRecordCollection =
        temporalRecordTuple.get(extendedOccurrenceTransform.getDataTag()).apply(Values.create());

    // STEP 4: Save to an avro file
    temporalRecordCollection.apply(
        WRITE_STEP, AvroIO.write(ExtendedOccurrence.class).to(targetDirectory));
